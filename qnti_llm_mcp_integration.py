...
def integrate_llm_with_qnti(qnti_system):
    ...
    try:
        ...
        def get_market_insight():
            """Generate AI market insight for dashboard using REAL market data"""
            try:
                ...
                try:
                    ...
                    technical_insights = [insight.get('insight', '') for insight in insights if insight.get('insight')]
                    
                    # Get market summary stats
                    market_summary = enhanced_intelligence.get_market_summary()
                    
                    # Get sample of real market data
                    symbols_data = {}
                    for symbol in ['EURUSD=X', 'GC=F', 'BTC-USD', '^GSPC'][:4]:
                        if symbol in enhanced_intelligence.market_data:
                            data = enhanced_intelligence.market_data[symbol]
                            tech = enhanced_intelligence.technical_indicators.get(symbol, {})
                            symbols_data[symbol] = {
                                'price': data.price,
                    ...
                    real_market_data = {
                        'total_insights': market_summary.get('total_insights', 0),
                        'high_priority_alerts': market_summary.get('high_priority_count', 0),
                        'avg_confidence': market_summary.get('average_confidence', 0.5),
                        'symbols_data': symbols_data
                    }
                except Exception as e:
                    logger.warning(f"Could not get enhanced market data: {e}")
                
                # Also get basic trading context
                ...
                market_insights_text = "\n".join(technical_insights[:5]) if technical_insights else "No enhanced insights available"
                
                symbols_summary = ""
                if real_market_data.get('symbols_data'):
                    for symbol, data in real_market_data['symbols_data'].items():
                        rsi_text = f"RSI: {data['rsi']:.1f}" if data.get('rsi') else ""
                        vol_text = f"Vol: {data['volatility']:.1f}%" if data.get('volatility') else ""
                        symbols_summary += f"• {symbol}: {data['change_percent']:+.2f}% {rsi_text} {vol_text}\n"
                
                prompt = f"""
                Analyze current market conditions using REAL market data and provide a trading insight.
                
                LIVE MARKET DATA:
                - Total Market Insights Available: {real_market_data.get('total_insights', 0)}
                - High Priority Alerts: {real_market_data.get('high_priority_alerts', 0)}
                - Average Confidence: {real_market_data.get('avg_confidence', 0.5):.2f}
                ...
                Based on the REAL market data above, provide a concise market insight (max 2 sentences) focusing on:
                1. Most significant market opportunity or risk right now
                2. Actionable advice based on current technical conditions
        ...
        def get_account_analysis():
            ...
            try:
                ...
                # Risk assessment - FIXED: High margin level = MORE available margin = GOOD
                risk_level = "Low"
                if margin_level > 1000:
                    risk_level = "Very Low"  # Lots of available margin = excellent
                elif margin_level > 500:
                    risk_level = "Low"       # Good available margin = safe
                elif margin_level > 200:
                    risk_level = "Medium"    # Moderate margin = caution
                else:
                    risk_level = "High"      # Low margin = risky (approaching margin call)
                ...
                IMPORTANT: Margin Level interpretation:
                - HIGH margin level (>1000%) = GOOD (lots of available margin)
                - LOW margin level (<200%) = BAD (approaching margin call risk)
                
                With margin level of {margin_level:.1f}%, this account has {"excellent" if margin_level > 1000 else "good" if margin_level > 500 else "moderate" if margin_level > 200 else "limited"} available margin.
                
                Provide a concise account analysis (max 2 sentences) focusing on:
                1. Account health status (considering the margin level correctly)
        ...
        def get_all_insights():
            ...
            try:
                ...
                # Get market insight using REAL live data
                try:
                    ...
                    try:
                        ...
                        # Get top insights
                        insights_data = enhanced_intelligence.get_insights(limit=5)
                        real_insights = [insight.get('insight', '') for insight in insights_data if isinstance(insight, dict) and insight.get('insight')]
                        
                        # Get key market data for major symbols
                        for symbol in ['EURUSD=X', 'GC=F', 'BTC-USD']:
                            if symbol in enhanced_intelligence.market_data:
                                data = enhanced_intelligence.market_data[symbol]
                                tech = enhanced_intelligence.technical_indicators.get(symbol, {})
                                rsi = getattr(tech, 'rsi', None)
                                ...
                                vol_status = "High" if volatility and volatility > 30 else "Low" if volatility and volatility < 15 else "Normal"
                                
                                key_symbols_data += f"{symbol}: {data.change_percent:+.2f}% RSI:{rsi_status} Vol:{vol_status}. "
                    except:
                        pass
                    ...
                    insights_text = " | ".join(real_insights[:3]) if real_insights else "Market monitoring active"
                    
                    prompt = f"""Analyze current market conditions using LIVE data.
                    
                    REAL MARKET CONDITIONS: {key_symbols_data}
                    CURRENT INSIGHTS: {insights_text}
                    ACTIVE TRADES: {len(trades)}
                    
                    Provide 1-2 sentence market insight based on REAL data."""
        ...
        @qnti_system.app.route('/api/ai/enhanced-market-analysis', methods=['GET'])
        def get_enhanced_market_analysis():
            """Generate comprehensive AI analysis of real market data"""
            try:
                ...
                try:
                    ...
                    # Get all insights
                    all_insights = enhanced_intelligence.get_insights(limit=50)
                    market_summary = enhanced_intelligence.get_market_summary()
                    
                    # Categorize insights
                    opportunities = [insight for insight in all_insights if 
                                   any(word in insight.get('insight', '').lower() for word in 
                                       ['buy', 'bullish', 'support', 'oversold', 'breakout'])]
                    
                    risks = [insight for insight in all_insights if 
                            any(word in insight.get('insight', '').lower() for word in 
                                ['sell', 'bearish', 'resistance', 'overbought', 'breakdown'])]
                    
                    # Get technical indicators summary
                    technical_data = {}
                    for symbol in ['EURUSD=X', 'GC=F', 'BTC-USD', '^GSPC', 'SI=F']:
                        if symbol in enhanced_intelligence.market_data:
                            data = enhanced_intelligence.market_data[symbol]
                            tech = enhanced_intelligence.technical_indicators.get(symbol, {})
                            ...
                            technical_data[symbol] = {
                                ...
                                'volatility': getattr(tech, 'volatility', None),
                                'bollinger_upper': getattr(tech, 'bollinger_upper', None),
                                'bollinger_lower': getattr(tech, 'bollinger_lower', None)
                            }
                    
                    market_analysis = {
                        'total_insights': len(all_insights),
                        'high_priority_count': market_summary.get('high_priority_count', 0),
                        'symbols_analyzed': len(enhanced_intelligence.market_data),
                    ...
                    logger.warning(f"Could not get enhanced market data for analysis: {e}")
                
                # Create comprehensive AI analysis prompt
                prompt = f"""
                Analyze comprehensive REAL market intelligence data and provide detailed trading insights.
                
                MARKET INTELLIGENCE SUMMARY:
                - Total Real-Time Insights: {market_analysis['total_insights']}
                - High Priority Alerts: {market_analysis['high_priority_count']}
                - Symbols Being Monitored: {market_analysis['symbols_analyzed']}
                
                TOP MARKET OPPORTUNITIES DETECTED:
                {chr(10).join([f"• {opp.get('insight', '')[:100]}..." for opp in market_analysis['top_opportunities'][:3]])}
                
                MAJOR RISK FACTORS IDENTIFIED:
                {chr(10).join([f"• {risk.get('insight', '')[:100]}..." for risk in market_analysis['major_risks'][:3]])}
                
                TECHNICAL INDICATORS STATUS:
                {chr(10).join([f"• {symbol}: {data['change_percent']:+.2f}% | RSI: {data.get('rsi', 'N/A')} | Vol: {data.get('volatility', 'N/A')}%" 
                             for symbol, data in list(market_analysis['technical_summary'].items())[:4]])}
                
                Provide a comprehensive market analysis (3-4 sentences) covering:
                1. Overall market sentiment and key themes
                2. Best trading opportunities based on technical data
                3. Primary risks to watch
                4. Recommended trading approach for next 24 hours
                
                Format: Direct professional analysis without "AI says" prefixes.
                """
                ...
                try:
                    ...
                    logger.error(f"Error generating enhanced market analysis: {e}")
                    analysis = "Enhanced market analysis temporarily unavailable. Real-time monitoring systems are active and collecting data."
                ...
                return jsonify({
                    ...
                    'timestamp': datetime.now().isoformat(),
                    'data_summary': {
                        'total_insights': market_analysis['total_insights'],
                        'opportunities_count': len(market_analysis['top_opportunities']),
                        'risks_count': len(market_analysis['major_risks']),
                        'symbols_monitored': market_analysis['symbols_analyzed']
                    },
                    'source': 'enhanced_market_intelligence'
                })
                
            except Exception as e:
                logger.error(f"Error in enhanced market analysis endpoint: {e}")
                return jsonify({
                    'success': False,
                    'analysis': 'Enhanced market analysis temporarily unavailable.',
                    'error': str(e)
                }), 500
