...
class QNTIMT5Bridge:
        ...
        try:
            if Path(self.config_file).exists():
                ...
                self.config = {
                    ...
                    "symbols": ["EURUSD", "GBPUSD", "USDJPY", "USDCHF", "AUDUSD", "USDCAD", "GOLD", "BTCUSD"],
                    "ea_monitoring": {
                        "check_interval": 30,
    ...
    def _sync_mt5_trades(self):
        """Sync MT5 trades with trade manager"""
        try:
            # Add debug logging to detect when trade_manager is corrupted
            if not hasattr(self, 'trade_manager'):
                logger.error("❌ DEBUG: trade_manager attribute missing from self")
                return
                
            if self.trade_manager is None:
                logger.debug("⚠️ DEBUG: trade_manager is None - skipping sync")
                return
                
            if isinstance(self.trade_manager, dict):
                logger.error(f"❌ DEBUG: trade_manager is a dict! Type: {type(self.trade_manager)}, Keys: {list(self.trade_manager.keys()) if isinstance(self.trade_manager, dict) else 'N/A'}")
                return
                
            if not hasattr(self.trade_manager, 'trades'):
                logger.error(f"❌ DEBUG: trade_manager has no 'trades' attribute! Type: {type(self.trade_manager)}, Attributes: {dir(self.trade_manager)}")
                return
            
            # Get positions and orders from MT5
            ...
            if positions is not None:
                for position in positions:
                    ...
                    if trade_id not in self.trade_manager.trades:
                        # Add new trade discovered from MT5
                        try:
                            trade = Trade(
                                trade_id=trade_id,
                                symbol=position.symbol,
                            ...
                            self.trade_manager.trades[trade_id] = trade
                            logger.info(f"Added MT5 trade to manager: {trade_id}")
                        except Exception as e:
                            logger.error(f"Error adding MT5 trade {trade_id}: {e}")
                
                # Check for closed trades
                for trade_id, trade in list(self.trade_manager.trades.items()):
                    if trade_id.startswith("MT5_") and trade.status == TradeStatus.OPEN:
                        # Check if trade still exists in MT5
                        mt5_ticket = int(trade_id.replace("MT5_", ""))
                        position_exists = any(pos.ticket == mt5_ticket for pos in positions)
                        
                        if not position_exists:
                            # Trade was closed in MT5, get deal info
                            try:
                                deals = mt5.history_deals_get(ticket=mt5_ticket)
                                if deals:
                                    for deal in deals:
    ...
    def _check_risk_management(self):
        """Check risk management parameters and apply controls"""
        try:
            # Add debug logging to detect when trade_manager is corrupted
            if not hasattr(self, 'trade_manager'):
                logger.error("❌ DEBUG: trade_manager attribute missing from self in risk management")
                return
                
            if self.trade_manager is None:
                logger.debug("⚠️ DEBUG: trade_manager is None in risk management - skipping")
                return
                
            if isinstance(self.trade_manager, dict):
                logger.error(f"❌ DEBUG: trade_manager is a dict in risk management! Type: {type(self.trade_manager)}, Keys: {list(self.trade_manager.keys()) if isinstance(self.trade_manager, dict) else 'N/A'}")
                return
                
            if not hasattr(self.trade_manager, 'trades'):
                logger.error(f"❌ DEBUG: trade_manager has no 'trades' attribute in risk management! Type: {type(self.trade_manager)}")
                return
                
            # Check if we have proper account info
            if not self.account_info:
                logger.debug("No account info available for risk management check")
                return
            
            # Ensure config is properly loaded
            if not hasattr(self, 'config') or not isinstance(self.config, dict):
                logger.warning("Config not properly loaded - skipping risk management check")
                return
            ...
            try:
                ...
                logger.debug(f"Error checking daily loss limit: {e}")
            
            # Check drawdown
            try:
                if (hasattr(self.account_info, 'equity') and hasattr(self.account_info, 'balance') and 
                    self.account_info.equity > 0 and self.account_info.balance > 0):
                    
                    drawdown = (self.account_info.balance - self.account_info.equity) / self.account_info.balance
                    
                    risk_config = self.config.get("risk_management", {})
                    emergency_drawdown = risk_config.get("emergency_close_drawdown", 0.20)
                ...
                logger.debug(f"Error checking drawdown: {e}")
    ...
    def _apply_ea_controls(self):
        ...
        try:
            # Check if trade_manager has ea_controls attribute
            if not hasattr(self.trade_manager, 'ea_controls'):
                logger.debug("Trade manager does not have ea_controls attribute - skipping EA controls")
                return
                
            # Check if ea_controls is None or empty
            if not self.trade_manager.ea_controls:
                logger.debug("No EA controls configured - skipping")
                return
            ...
            for ea_name, control in self.trade_manager.ea_controls.items():
                ...
                if action == 'pause':
                    ...
                    logger.info(f"EA {ea_name} paused and all trades closed")
    ...
    def get_account_info(self) -> Optional[Dict]:
        """Get current account information as dictionary"""
        try:
            if self.account_info:
                return {
                    ...
                    'margin_call': self.account_info.margin_call,
                    'margin_stop_out': self.account_info.margin_stop_out
                }
            return None
        except Exception as e:
    ...
    def get_open_positions(self) -> List[Dict]:
        """Get current open positions as list of dictionaries"""
        try:
            positions = []
            if self.connection_status == MT5ConnectionStatus.CONNECTED:
                ...
                if mt5_positions:
                    for pos in mt5_positions:
                        positions.append({
                            ...
                            'magic': pos.magic,
                            'comment': pos.comment,
                            'time': datetime.fromtimestamp(pos.time) if pos.time else None
                        })
            return positions
        except Exception as e:
